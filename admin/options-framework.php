 ...
 * Updates:
 *
 * 10/11/11 20:54 - Modified optionsframework_validate() to check if the option
 * should pass through a sanitization filter or not. this is done by checking if
 * $option['sanitize'] is specified and set to FALSE. This is to make it
 * possible to return unsanitized values.
 *
 * 10/11/11 - modified optionsframework_add_page() to use add_menu_page() and
 * changed the page title and menu title. Also added a menu icon and set the
 * menu position to display right after the Appearance menu (position 61).
 */
...
function optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['update'] ) ) {
		...
		foreach ( $options as $option ) {
			...
			//check if option should pass through sanitization filter or not
			if( isset( $option['sanitize'] ) && false === $option['sanitize'] ){
				//do this only when sanitize is set explicitly and it is false
				$clean[$id] = $input[$id]; //return unsanitized
			} else {
